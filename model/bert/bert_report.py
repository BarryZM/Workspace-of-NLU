...
pred_path = os.path.join(base_dir, 'output/BERT/BERT_maxLen128/result/test_predictions.tsv')
output_path = os.path.join(base_dir, 'output/BERT/BERT_maxLen128/result')

parser = argparse.ArgumentParser(description='Process some integers.')
parser.add_argument('--label', '-l', dest='labelPath', action='store', help='path to testset', default=label_path)
parser.add_argument('--pred', '-p', dest='predPath', action='store', help='path to prediction', default=pred_path)
parser.add_argument('--output', '-o', dest='outPath', action='store', help='path to results report', default=output_path)
