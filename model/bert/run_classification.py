...
from model.bert import modeling, optimization, tokenization
...
flags.DEFINE_string("train_file", "aaa.txt", " train data file name")
flags.DEFINE_string("dev_file", "aaa.txt", " dev data file name")
flags.DEFINE_string("test_file", "aaa.txt", " test data file name")

flags.DEFINE_string("EPOCH", "5", " epoch for train")
# data/stacking_data-3/a-2048.txt
flags.DEFINE_string(
...
flags.DEFINE_string(
    "bert_config_file", os.path.join(base_dir, "model/bert/chinese_L-12_H-768_A-12/bert_config.json"),
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("task_name", "domainclf", "The name of the task to train.")

flags.DEFINE_string("vocab_file", os.path.join(base_dir, "model/bert/chinese_L-12_H-768_A-12/vocab.txt"),
...
flags.DEFINE_string(
    "init_checkpoint", os.path.join(base_dir, "model/bert/chinese_L-12_H-768_A-12/bert_model.ckpt"),
...
class DomainClf_Processor(DataProcessor):
    ...
    def get_real_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, FLAGS.real_test_file)), "real_test")
    ...
    def get_labels(self):
        ...
        labels, _, _ = read_label_from_file(os.path.join(base_dir, 'output/label-THU.txt'))
        print("label", labels)
        return labels
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            label_txt = line[0].replace("__label__", "")
...
def parameter_setting(m_type='', data_path='', train_file='', dev_file='', test_file='', max_seq_length=64, EPOCH=3):
    FLAGS.train_file = train_file
    FLAGS.dev_file = dev_file
    ...
    print("\n\n output dir is {} \n\n".format(FLAGS.model_output_dir))


def train_eval_bert(m_type, data_path, train_data_file, dev_data_file, seq_len, epoch):
    """ parameter setting """
    parameter_setting(m_type=m_type, data_path=data_path, train_file=train_data_file, dev_file=dev_data_file,
                      max_seq_length=seq_len, EPOCH=epoch)
    """ logging setting """
    ...
    early_stopping = tf.contrib.estimator.stop_if_no_decrease_hook(
        ...
        min_steps=num_batch_single_epoch * 1)  # min_step=num_batch_single_epoch*5)  # min_step
    """
    train and eval
    """
    train_spec = tf.estimator.TrainSpec(train_input_fn, hooks=[early_stopping])
    eval_spec = tf.estimator.EvalSpec(eval_input_fn, exporters=None, throttle_secs=1)  # throttle_secs 多少秒之内不进行 eval
    result_eval = tf.estimator.train_and_evaluate(estimator, train_spec, eval_spec)

    print('\n\n result eval', result_eval)
    # 'eval_accuracy': 0.93066406, 'eval_loss': 0.8050002, 'loss': 0.8050002, 'global_step': 2404


def save_pb_bert(ckpt_folder):
    """
    save pb model
    """
    ...
    print("=" * 100 + "\n\n\n\n\n Save best checkpoint to pb")

    model_file = tf.train.latest_checkpoint(ckpt_folder)  # use the latest_checkpoint as the best model, need to review
    # saver.restore(sess, model_file)
    sess = tf.Session(config=config)
    graph = tf.get_default_graph()
    with graph.as_default():
        input_ids_ = tf.placeholder(tf.int32, [None, FLAGS.max_seq_length], name="input_ids")
        input_mask_ = tf.placeholder(tf.int32, [None, FLAGS.max_seq_length], name="input_mask")
        label_ids_ = tf.placeholder(tf.int32, [None], name="label_ids")
        segment_ids_ = tf.placeholder(tf.int32, [FLAGS.max_seq_length], name="segment_ids")
        loss, per_example_loss, logits, probabilities = create_model(bert_config, False, input_ids_,
                                                                     input_mask_,
                                                                     segment_ids_,
                                                                     label_ids_, len(label_list),
        ...
        with tf.gfile.FastGFile(os.path.join(FLAGS.model_output_dir, 'bert_pb.pb'), 'wb') as f:
            f.write(constant_graph.SerializeToString())


def predict_bert(m_type, data_path, train_data_file, test_data_file):

    parameter_setting(m_type=m_type, data_path=data_path, train_file=train_data_file, test_file=test_data_file)
    ...
    predict_file = os.path.join(FLAGS.model_output_dir, "predict.tf_record")  # overwrite the latest file
