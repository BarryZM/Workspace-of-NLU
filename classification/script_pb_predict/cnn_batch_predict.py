...
parser.add_argument('--embedding', type=str, default=base_dir + '/data/vocab_and_embedding_new.pkl')
...
with tf.Graph().as_default():
    ...
    word_to_id, _ = read_vocab_and_embedding_from_pickle_file(args_in_use.embedding)
    ...
    with tf.Session() as sess:
        ...
        y_pred_cls = []
        for x, y in batch_iter_x_y(x_encode, y_test_cls, 1024):
            feed_dict = {
                test_text: x,
                keep_prob: 1.0
            }
        ...
        """
        # output_result_path = os.path.join(base_dir, 'results/cnn')
        #
        ...
        log_dir = os.path.join(base_dir, 'results', 'cnn-logs')
        ...
        for i in tqdm(range(len(y_test_cls))):
            ...
            print(str(testdata[i].strip())+'\t'+ str(y_pred_cls[i]) + '\t' +  str(y_test_cls[i]) + '\t' + is_sucess)
        f.close()

        print('=====testing=====')
        target_idx = set(list(set(y_pred_cls)))
